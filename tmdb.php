...
error_reporting(0);
	...
	if($_POST['submit'] != ""){
		...
		$imdbid = $_POST['imdbid'];
		$tmdbid = $_POST['idmuvi-core-tmdbid-value'];
				...
				(function( $ ) {
					...
					$('document').ready(function(){
						...
						$('input[name=idmuvi-ret-gmr-button]').click(function() {
							...
							var valImdbiid = $('input[name=tmdbID]').get(0).value;
							...
							$.getJSON( "https://api.themoviedb.org/3/movie/" + valImdbiid + "?append_to_response=videos,keywords,images,credits,release_dates" + languange + apikey, function(json) {
								...
								$.each(json, function(key, val) {
									...
									if(key == "imdb_id"){
										// Remove min value
										valImdbID+= ""+val+"";
										$('input[name=imdbid]').val(valImdbID);
									}
									...
									if(key == "production_companies"){
										$.each(json.production_companies, function(i, item) {
									...
									var valImg = "";
									if(key == "poster_path"){
										valImg+= "https://image.tmdb.org/t/p/w200"+val+"";
		...
		<div id="col-container" style="width:50%; float:left;">
			<div class="metabox-holder idmuvi-core-metabox-common-fields">
				...
				<h3 class="nav-tab-wrapper">
					<a class="nav-tab nav-tab-active" id="muvi-settings-tab" href="/tmdb2.php">Back To TV-Series</a>
				</h3>
...
<form action="" method="POST">
					...
					<p>
						<label for="opsi-title"><strong>Title:</strong></label>
					...
					<p>
						<label for="opsi-title"><strong>Genre:</strong></label>
						<input type="text" class="regular-text display-block" id="opsi-title" name="genre" value="" />

					</p>
					<p>
						<label for="opsi-title"><strong>Writer:</strong></label>
					...
					<p>
						<label for="opsi-runtime"><strong>Runtime:</strong></label>
					...
					<p>
						<label for="opsi-release"><strong>Release Date:</strong></label>
						<input type="text" class="regular-text display-block" id="opsi-release" name="idmuvi-core-released-value" value="" />

					</p>
					<p>
						<label for="opsi-title"><strong>Production:</strong></label>
					...
					<p>
						<label for="opsi-tmdbid"><strong>ImdbID:</strong></label>
						<input type="text" style="max-width:120px" class="regular-text display-block" id="opsi-tmdbid" name="imdbid" value="" />
...
echo '<br>Runtime: '.$runtime.' Menit';
echo '<br>Country: '.$country;
echo '<br>Languange: '.$languange;
echo '<br>Release Date: '.$release;
echo '<br>Production: '.$network;
echo '<br>TmdbRating: '.$rating.'/'.$votes;
echo '<br>IMDB/TMDB: <a href="https://www.imdb.com/title/'.$imdbid.'/" target="_blank">'.$imdbid.'</a> / <a href="https://www.themoviedb.org/movie/'.$tmdbid.'/" target="_blank">'.$tmdbid.'</a>';
echo '<br>Trailer: '.$youtube;
echo '<img src="'.$poster.'" width="200px" class="img-responsive" style="display:inherit !important;">';
